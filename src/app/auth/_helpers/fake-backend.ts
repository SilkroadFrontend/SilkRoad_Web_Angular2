    ...
    backend.connections.subscribe((connection: MockConnection) => {
        ...
        setTimeout(() => {
            ...
            if (connection.request.url.endsWith('/api/login') && connection.request.method === RequestMethod.Post) {
                ...
                // default account
                if (params.account === '11246' && params.password === '123456') {
                    filteredUsers[0] = {
                        fullName: '唐华宇',
                        account: '11246',
                        password: '123456',
                ...
                if (filteredUsers.length) {
                    ...
                    connection.mockError(new Error('Account or password is incorrect'));
                }
            ...
            if (connection.request.url.endsWith('/api/users') && connection.request.method === RequestMethod.Post) {
                ...
                if (duplicateUser) {
                    return connection.mockError(new Error('Account "' + newUser.account + '" is already registered'));
                }
            ...
            if (connection.request.url.endsWith('/api/forgot-password') && connection.request.method === RequestMethod.Post) {
                ...
                // find if any user matches login credentials
                let filteredUsers = users.filter(user => {
                    return user.account === params.account;
                });

                if (filteredUsers.length) {
                    // in real world, if account is valid, send account change password link
                    let user = filteredUsers[0];
                    connection.mockRespond(new Response(new ResponseOptions({ status: 200 })));
                } else {
                    // else return 400 bad request
                    connection.mockError(new Error('User with this account does not exist'));
                }
